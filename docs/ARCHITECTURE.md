...
初回訪問時は目的・収集項目・保存期間を表示し、利用者が「アクセス解析を許可」するまでGoogle Analyticsのスクリプトを読み込みません。「利用しない」を選べるほか、フッターの「アクセス解析の設定」からいつでも無効化できます。無効化時はローカルストレージに記録し、GAの無効化フラグと同意状態を更新して以後のイベント送信を止めます。実装は `src/components/privacy/AnalyticsConsent.tsx` です。
