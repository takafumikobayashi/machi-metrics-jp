...
アクセス解析は、次の条件を満たしたうえで導入します。目的はサイトの利用状況を把握してグラフ・ページ構成を改善することに限り、広告配信や個人の評価には使いません。収集するのはページURL・ページタイトル・参照元・アクセス日時と、Google Analyticsが生成するブラウザ・端末等の技術情報です。送信イベントはページ表示です。ただしGA4の拡張計測機能は `gtag('config')` から制御できず、オンのままだとスクロールや外部リンクのクリックなども自動送信されます。そのため、管理画面でオフにしたかどうかを `enhancedMeasurementDisabled` として宣言し、画面の説明文をその値から組み立てます（[[D-025]]）。既定はオフにしていない状態です。いずれの設定でも、入力値や氏名・住所・メールアドレスは送信しません。Google Analyticsのイベントデータ保持期間は管理画面で14か月を上限に設定し、本プロジェクト側では解析データを独自保存しません。同意状態はブラウザのローカルストレージに保存します。

初回訪問時は目的・収集項目・保存期間を表示し、利用者が「アクセス解析を許可」するまでGoogle Analyticsのスクリプトを読み込みません。「利用しない」を選べるほか、フッターの「アクセス解析の設定」からいつでも無効化できます。無効化時はローカルストレージに記録し、GAの無効化フラグと同意状態を更新して以後のイベント送信を止めます。さらに、`NEXT_PUBLIC_GA_MEASUREMENT_ID` を正規の本番デプロイ環境だけに設定し、プレビュー・ステージングでは未設定にします。`NODE_ENV=production` だけではデプロイ先を判定できないため、測定IDが未設定の環境では同意パネルもスクリプトも出しません。実装は `src/components/privacy/AnalyticsConsent.tsx` と `src/lib/site/analytics.ts` です。
