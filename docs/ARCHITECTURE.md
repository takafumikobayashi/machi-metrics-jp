...
アクセス解析は、次の条件を満たしたうえで導入します。目的はサイトの利用状況を把握してグラフ・ページ構成を改善することに限り、広告配信や個人の評価には使いません。収集するのはページURL・ページタイトル・参照元・アクセス日時と、Google Analyticsが生成するブラウザ・端末等の技術情報です。送信イベントはページ表示だけとし、入力値や氏名・住所・メールアドレスは送信しません。Google Analyticsのイベントデータ保持期間は管理画面で14か月を上限に設定し、本プロジェクト側では解析データを独自保存しません。同意状態はブラウザのローカルストレージに保存します。
